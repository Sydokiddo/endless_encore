...
// List of Blocks:
    ...
    requiresCorrectToolForDrops().destroyTime(0.3f).strength(0.3f).lightLevel((blockStatex) -> {
    return 15;
    })));
    ...
    .destroyTime(0.1f).strength(0.1f).noOcclusion().lightLevel((blockStatex) -> {
    return 10;
    })));
    ...
    .sound(SoundType.WOOL).strength(1.0f).lightLevel((blockStatex) -> {
    return 1;
    })));
